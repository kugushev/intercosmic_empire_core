use std::ptr;
use glam::Vec3;
use intercosmic_empire::ffi::{ice_battle_update, ice_build_route_add_waypoint, ice_build_route_new, ice_get_battle_view_model, ice_init_game, ice_register_planet, ice_register_stellar_system, ice_spawn_spaceship, ice_start_battle};
use intercosmic_empire::ffi_models::{FFILog, FFIOutcome, RouteBuildersSource};
...
fn spawn_spaceship_start_translation_is_land() {
    ...
    let mut elapsed = 0.0;
    while elapsed < 10.0 {
        ice_battle_update(game_context, DELTA_TIME_120_FPS, FFILog::default()).assert(FFIOutcome::Ok);
