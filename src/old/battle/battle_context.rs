use crate::old::battle::ecs_context::EcsContext;
use crate::old::battle::models::battle_parameters::BattleParameters;
use crate::old::battle::models::battle_state::BattleState;
use interoptopus::ffi_type;
use crate::old::battle::factories::spaceship_factory::create_spaceship;
use crate::old::battle::models::route::Route;
use crate::old::battle::models::warp_gate::WarpGate;
use crate::old::battle::utils::ecs_ext::resources_and_non_send_mut;
use crate::old::battle::views::BattleViewsState;
...
use crate::old::core::models::stellar_system::StellarSystem;
    ...
    pub fn spawn_spaceship(&mut self, faction: &Faction, spawner_id: i32, route: Route,
        ...
        let success = match found {
            Some(p) => {
                ...
                trace_old!(logger, format!("Produced={produced} from planet {} {}/{}", p.info.id.0, p.current_product, parameters.cost));
                produced
            }
            None => {
                ...
                trace_old!(logger, format!("Produced={produced} from warp gate {id} {}/{}", warp_gate.current_product, parameters.cost));
                produced
            }
        };

        if success {
            trace_old!(logger, format!("Create spaceship mark {} {}", mark.clone() as i32, faction.clone() as i32));
